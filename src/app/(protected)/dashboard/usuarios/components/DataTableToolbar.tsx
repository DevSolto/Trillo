  ...
  async function handleBulkAction(value: string) {
    ...
    try {
      if (value === 'delete') {
        ...
        notify({ type: 'success', title: 'Usuários', message: 'Itens excluídos com sucesso.' })
      } else if (value.startsWith('role:')) {
        const role = value.split(':')[1] as 'admin' | 'editor'
        await Promise.all(selectedIds.map((id) => updateUser(id, { role })))
        notify({ type: 'success', title: 'Usuários', message: 'Perfil atualizado.' })
      }
      router.refresh()
    } catch (e: unknown) {
    ...
    <div className="flex items-center justify-between">
      <div className="flex flex-1 items-center gap-2">
        ...
        {table.getColumn("role") && (
          <DataTableFacetedFilter column={table.getColumn("role")!} title="Perfil" options={roles} />
        )}
        {isFiltered && (
